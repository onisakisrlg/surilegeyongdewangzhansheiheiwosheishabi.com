...
import { MOCK_PROJECTS, MOCK_PACKAGE_ORDERS, ADDONS_LIST, LOGISTICS_ROUTES } from './constants';
...
export default function App() {
  ...
  const [appStep, setAppStep] = useState<'list' | 'experience' | 'priority' | 'addons' | 'logistics'>('list');
  const [packExp, setPackExp] = useState<'novice' | 'expert' | null>(null);
  const [packPriority, setPackPriority] = useState<'weight' | 'volume' | 'balanced' | null>(null);
  const [packAddons, setPackAddons] = useState<string[]>([]);
  const [packLogistics, setPackLogistics] = useState<string | null>(null);
                    ...
                    >
                      ...
                      <div className="flex-1 text-left min-w-0">
                        ...
                        <div className="flex items-center justify-between mt-0.5">
                          <p className="font-bold text-sm truncate flex-1 pr-2">{project.name}</p>
                          {project.status === 'completed' && (
                            <span className={`shrink-0 flex items-center gap-0.5 text-[9px] font-bold px-1.5 py-0.5 rounded-sm border ${
                              selectedId === project.id 
  ...
  <div className="flex flex-col h-full relative">
    {/* App Header (System Bar & Navbar) */}
                                ...
                                {/* Search & Filter */}
                                <div className="bg-[#f7f7f7] px-3 py-3 flex gap-2 shrink-0 relative z-10 border-b border-gray-100">
                                  <div className="flex-1 bg-white rounded-full flex items-center px-4 h-9 shadow-[inset_0_1px_3px_rgba(0,0,0,0.02)] border border-gray-100/50">
                                    <input 
                                      type="text" 
                                  ...
                                  <button className="bg-white rounded-full h-9 px-4 flex items-center gap-1.5 shadow-[inset_0_1px_3px_rgba(0,0,0,0.02)] border border-gray-100/50 active:scale-95 transition-transform text-sm text-gray-600 font-medium">
                                    筛选 <span className="text-rose-400 text-xs font-bold border-l border-gray-200 pl-1.5 ml-0.5">全部平台/日期</span>
                                  </button>
                                </div>
                                      ...
                                      <div key={order.id} className="bg-white m-3 rounded-xl shadow-[0_2px_8px_rgba(0,0,0,0.04)] overflow-hidden">
                                        ...
                                        <div className="p-3 flex items-center gap-3">
                                          ...
                                          <div className="w-24 h-24 rounded-lg bg-gray-100 relative overflow-hidden shrink-0 border border-gray-100">
                                            ...
                                            />
                                            <div 
                                              className="absolute top-0 left-0 text-white text-[10px] px-1.5 py-0.5 font-bold rounded-br-lg shadow-sm"
                                          ...
                                          <div className="flex-1 flex flex-col justify-between h-24 py-0.5">
                                            <div>
                                              <h3 className="text-[13.5px] font-bold text-gray-800 leading-snug line-clamp-2">
                                                {order.title}
                                              </h3>
                                              <div className="mt-1">
                                ...
                                <div className="absolute bottom-[96px] right-4 z-20">
                                  <button className="bg-[#ffd200] text-[#8a6800] text-[13px] font-bold px-4 py-2 rounded shadow-md active:scale-95 transition-transform">
                                    单件速发
                                  </button>
                                </div>
                                ...
                                <div className="absolute bottom-0 left-0 right-0 bg-white border-t border-gray-100 shadow-[0_-4px_16px_rgba(0,0,0,0.03)] z-30 pb-safe">
                                  <div className="px-4 py-2 flex items-center justify-between border-b border-gray-50">
                                    ...
                                    <span className="text-[13px] font-medium text-gray-800">已选总重: <span className="font-mono">{selectedTotalWeight}克</span></span>
                                  </div>
                                  <div className="px-4 py-2 flex items-center justify-between gap-3">
                                    ...
                                    <div className="flex gap-2.5">
                                      <button 
                                        onClick={() => setAppStep('experience')}
                                        className="px-5 py-2.5 rounded-full bg-[#ffd200] text-gray-900 text-sm font-bold shadow-sm active:scale-95 transition-transform"
                                      >
                                        发起直邮
                                      </button>
                                      <button 
    ...
    <div className="bg-white p-4 border-t border-gray-200 absolute bottom-0 left-0 right-0 z-30 pb-safe shadow-[0_-4px_16px_rgba(0,0,0,0.03)] shrink-0">
      ...
      >
        确认提交打包
                          ...
                          {/* Right Column: Management Backend View - Admin */}
                          <div className="flex-1 overflow-y-auto bg-white p-4 md:p-8 space-y-6 relative">
                            ...
                            <div className="bg-gray-50 rounded-xl p-8 text-center border-2 border-dashed border-gray-200">
                              ...
                              <h3 className="text-sm font-bold text-gray-600 mb-1">后台处理</h3>
                              <p className="text-xs text-gray-500">管理员在这里查看用户的打包请求并进行处理配置</p>
                            </div>
                          </div>
                        </div>
                      ) : selectedId === '20260515' ? (
