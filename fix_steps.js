...
// The sed commands replaced some but not all accurately because of the specific strings. Let's fix them properly.

// First, for step 4 (the former step 3)
...
// Step 4 (增值服务)
// This used to be step 4, but sed changed its wrappers. Let's find it.
// It has "Step 4: 其他增值服务"
...
// In Step 4 (former Step 3, 拆除包装要求), change the number from '3' to '4'.
...
// Also fix `Step 3: 拆包要求` to `Step 4: 拆包要求`
code = code.replace(/\{\/\* Step 3: 拆包要求 \*\/\}/g, `{/* Step 4: 拆包要求 */}`);

// We need to inject the new Step 3: 控制重量/体积 right before Step 4.
// Let's locate the end of Step 2:
//               {/* Step 4: 拆包要求 */}
const newStep3 = `
              {/* Step 3: 控制重量/体积 */}
              {formActiveStep >= 3 && (
                <div ref={step3Ref} className={\`bg-white rounded-xl shadow-sm border overflow-hidden transition-all duration-300 \${formActiveStep === 3 ? 'border-blue-400 ring-2 ring-blue-100' : 'border-gray-200'}\`}>
                  <div className="px-4 py-3 bg-gray-50 border-b border-gray-100 flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      ...
                      <h2 className="font-bold text-[14px] text-gray-800">控制重量/体积要求</h2>
                    </div>
                    {formActiveStep > 3 && (
