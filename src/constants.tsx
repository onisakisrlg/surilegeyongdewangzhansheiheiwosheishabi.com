...
export const MOCK_PROJECTS: PrototypeProject[] = [
  {
    ...
    name: "通用支付强制支付",
    category: "账单与支付",
    description: "针对【退运邮费】、【入库到付】、【包裹税金】等高频仓储关税场景，适配苹果/谷歌商店合规规范，构建【APP端强制登出与登录限制引导 + 微信公众号H5强制支付与逐单结算】全流程解决方案。",
    previewUrl: null,
    redmineUrl: "https://redmine.rakutao.vip/issues/16332",
    status: "in-progress",
    specs: [
      {
        title: "一、 需求背景与合规架构定位 (App Store / Google Play 商店合规)",
        content: "1. 业务痛点与防坏账需求：用户包裹退回产生【退运邮费】、到仓【入库到付】代垫快递费、海关查验【包裹税金】及超期滞仓费，过去因部分用户失联或拒付导致仓库垫付资金沦为坏账亏损。\n2. 应用商店合规红线 (核心架构升级)：\n   - 因 Apple App Store 与 Google Play 应用商店政策严格限制应用内非IAP强制锁屏支付，原生APP端不得直接弹出全屏强制支付拦截界面。\n   - 规范调整为：【APP端强制登出 + 登录拦截限制提示 + 引导微信公众号【乐酷淘】H5登录锁定强制支付】，既彻底规避应用商店下架与审核风险，又有效保障仓库款项催收与防坏账闭环。"
      },
      {
        title: "二、 管理端（后台）账单创建、到付触发与智能联动",
        content: "1. 入库到付自动触发规则：仅当仓库入库人员设置到付金额并成功完成入库后，系统自动针对该用户ID创建一个【通用支付】（默认开启强制支付模式），并自动同步上传到付面单/凭证图片。除入库到付成功场景外，其他所有费用场景（退运邮费、包裹税金、滞仓费等）均由管理人员人工手动创建。\n2. 高频预设与智能联动：\n   - 置顶三大高频快捷预设：【退运邮费】（退回包裹运费）、【入库到付】（代垫快递费）、【包裹税金】（海关清关关税补缴）。\n   - 智能联动模式：管理员点击或选择置顶三大预设时，系统智能自动选中【强制支付】拦截模式（无倒计时常规必付）；非强制支付模式下统一固定默认 24 小时倒计时（超时自动作废）。\n3. 唯一值精准搜索与详细输入栏：\n   - 订单列表顶部新增【详细输入栏 / 唯一值搜索框】，支持对主订单号 (LO/OR)、子单号、支付单号 (CO) 及会员ID等唯一标识精准一键检索。\n4. 凭证上传与快捷粘贴 (Ctrl+V)：限制上传 1 张高清凭证图（海关官方税单/快递到付面单/称重图），支持鼠标点击上传及直接 Ctrl+V / Cmd+V 快速粘贴剪贴板图片，秒级完成开单。"
      },
      {
        title: "三、 APP端与公众号【乐酷淘】H5端落地规范",
        content: "1. APP端逻辑（强制登出与登录限制提示）：\n   - 触发强制支付时，APP端立即将该账号强制登出（清除登录态凭证）。\n   - 用户在APP端尝试登录时，系统拦截登录并弹出弹窗提示：“账号限制，请去公众号【乐酷淘】选择H5登录处理。”并提供一键复制公众号名称功能。\n2. 公众号【乐酷淘】H5端逻辑（锁定页面强制支付）：\n   - 用户进入微信公众号【乐酷淘】并选择H5登录后，H5页面展示全屏锁定强制支付卡片。\n   - 严禁合并支付：多笔欠款时必须按“一单一单独立卡片”逐单展示与独立支付，方便用户逐笔核对凭证单据。每支付成功一笔，页面自动切换至下一笔待支付卡片，全部结清后自动解除限制。\n   - 透明举证与凭证预览：展示日元待付金额（含参考折合RMB提示）、费用产生时间、详细原因说明及单张官方凭证原图（支持点击全屏放大预览）。\n   - 纯文字多渠道结算：支持【微信支付】、【支付宝】、【预存金支付】、【银联支付】4种标准渠道，默认不勾选任何支付方式，由用户自主选择。\n   - 申诉与客服缓冲通道：提供【账单有异议？一键联系客服 / 发起申诉】入口，申诉核对期间订单切为 [申诉审核中]，临时解除锁定，防止错单死锁。"
      },
      {
        title: "四、 多阶通知预警与跨端提醒闭环",
        content: "1. 即时多级推送：通用支付账单生成后，系统下发微信公众号模板消息通知、APP Push 消息与站内信通知，明细包含账单标题、日元金额与H5处理引导。\n2. 账号状态实时联动：在H5端完成全部强制支付后，系统毫秒级更新状态为 [已支付]，账号限制立即解除，用户可重新正常登录APP端进行正常出库与操作。"
      },
      {
        title: "五、 异常闭环、申诉审核与解封逻辑",
        content: "1. 实时自动解封闭环：在H5端完成支付后，系统实时解除账号限制，APP端与H5端均畅通交易与出库流程。\n2. 申诉审核闭环：\n   - 申诉通过：客服/管理员可一键作废或调整账单金额，释放锁定；\n   - 申诉驳回：系统附带明确驳回理由下发通知，重新恢复限制状态。\n3. 紧急人工特权解封与溯源：管理端提供主管特权一键撤销/作废功能，操作日志全链路留存，满足风控与合规审计需求。"
      }
    ]
  },
  {
