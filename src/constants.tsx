...
export const MOCK_PROJECTS: PrototypeProject[] = [
  {
    ...
    specs: [
      {
        title: "一、 需求背景与核心目标",
        content: "1. 业务痛点：用户包裹因退回产生【退运邮费】、包裹到仓产生的【入库到付】垫付款、以及海关清关查验产生的【包裹税金】或超期仓储费，需由仓库向指定用户发起通用补缴账单。过去部分失联或放弃包裹的用户存在欠款拒付风险，导致仓库亏损。\n2. 核心目标：建立管理端灵活创建账单能力及入库到付自动触发机制，并在APP端实现【强制支付】硬拦截规则，确保高频欠费即时扣缴并恢复业务出库。"
      },
      {
        title: "二、 管理端（后台）与入库到付触发规范",
        content: "1. 入库到付自动触发：当入库订单存在“到付”（仓库代垫快递费）时，系统将自动为用户创建【通用支付】账单，并默认采用【强制支付】逻辑。\n2. 独立展示不合并：即便同一用户存在多个到付订单，系统严禁合并合并为一个大账单，必须一单一单单独展示、单独生成通用支付记录，供用户在APP端逐单核对与独立完成支付。\n3. 会员ID录入与唯一值搜索：\n   - 管理端订单列表新增【详细输入栏 / 唯一值搜索框】，支持输入主订单号 (LO/OR)、子单号、支付单号 (CO) 或会员ID等唯一值精准搜索。\n4. 快捷预设与置顶：\n   - 置顶三大高频快捷预设：【退运邮费】、【入库到付】、【包裹税金】，点击后自动选中【强制支付】拦截模式（锁定APP全屏）。\n5. 倒计时固定规范：非强制支付模式下无需人工选择倒计时时长，系统统一默认设置为 24 小时倒计时（超时自动作废）。\n6. 凭证图片上传（仅限1张 & 支持粘贴）：限制上传 1 张凭证图片，支持在弹窗内直接按 Ctrl+V / Cmd+V 快捷粘贴。"
      },
      {
        title: "三、 APP移动端展示与逐单支付规范",
        content: "1. 全屏卡片展示：强制支付模式下，APP进入全屏账单待支付状态。多笔强制支付时一单一单独立展示，用户需逐单完成支付。\n2. 金额与详情列示：仅展示待支付日元金额（¥1,249 日元），隐藏折合人民币；展示详细费用说明及单张凭证（点击放大预览）。\n3. 头部信息精简：顶部仅展示【待支付】状态标签，隐藏冗余的费种分类标签，不显示关联合约主单号，保持页面整洁。\n4. 支付渠道选择：提供【微信支付】、【支付宝支付】、【预存金支付】、【银联支付】4种纯文字标准渠道，默认选中【微信支付】。\n5. 支付解除：用户完成单笔支付后，若还有其他待付强制账单则继续展示下一单；全部支付完成后系统即时恢复APP正常功能。"
      },
      {
        title: "四、 异常处理与账单撤销流程",
